...
var _ = ginkgo.Describe("context token resolution", func() {
	...
	ginkgo.It("uses embedded OIDC tokens before starting OIDC login", func() {
		...
		ctx := &mccontext.MCContext{Name: "local", Server: server, OIDC: &oidcclient.Tokens{AccessToken: "stored-token"}}
	...
	ginkgo.It("starts OIDC login when no usable token is available", func() {
		...
		ctx := &mccontext.MCContext{Name: "local", Server: "http://mission-control.local"}
	...
	ginkgo.It("starts OIDC login against the frontend URL for resolved /api contexts", func() {
		...
		ctx := &mccontext.MCContext{Name: "local", Server: "http://mission-control.local/api"}
		Expect(EnsureContextToken(&cobra.Command{}, ctx, io.Discard)).To(Succeed())
		Expect(ctx.OIDC.AccessToken).To(Equal("oauth-token"))
	...
	ginkgo.It("uses a manual context token when no OIDC tokens are configured", func() {
		ctx := &mccontext.MCContext{Name: "user-b", Server: "http://mission-control.local", Token: "user-b.jwt.token"}
		token, err := mccontext.ResolveContextToken(ctx)
	...
	ginkgo.It("uses embedded OIDC tokens for same-server contexts", func() {
		server := "http://mission-control.local"
		cfg := &mccontext.MCConfig{Contexts: []mccontext.MCContext{
...
var _ = ginkgo.Describe("context remove", func() {
	...
	ginkgo.It("clears the current context when no contexts remain", func() {
		Expect(mccontext.SaveConfig(&mccontext.MCConfig{
			CurrentContext: "local",
			Contexts:       []mccontext.MCContext{{Name: "local", Server: "http://local"}},
		})).To(Succeed())
...
var _ = ginkgo.Describe("API base resolution", func() {
	...
	ginkgo.It("prefers the frontend /api health endpoint", func() {
		...
		resolved, err := mccontext.ResolveAPIBase(server.URL)
	...
	ginkgo.It("persists the canonical API base after an HTTP to HTTPS redirect", func() {
		...
		resolved, err := mccontext.ResolveAPIBase(frontend.URL)
	...
	ginkgo.It("rejects HTTPS to HTTP health redirects", func() {
		...
		_, err := mccontext.ResolveAPIBase(secure.URL)
	...
	ginkgo.It("does not follow redirects on authenticated API requests", func() {
		...
		response, err := mccontext.NewAPIClientForServer(server.URL, "secret-token").

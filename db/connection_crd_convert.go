...
func connectionFromCRDSpec(obj *v1.Connection, dbObj *models.Connection) {
	if obj.Spec.AWS != nil {
	...
	if obj.Spec.Anthropic != nil {
		...
		if obj.Spec.Anthropic.Model != nil {
			dbObj.Properties = map[string]string{"model": *obj.Spec.Anthropic.Model}
		}
	}
	...
	if obj.Spec.OpenAI != nil {
		...
		if obj.Spec.OpenAI.Model != nil {
			dbObj.Properties = map[string]string{"model": *obj.Spec.OpenAI.Model}
		}
	}
	...
	if obj.Spec.Ollama != nil {
		...
		if obj.Spec.Ollama.Model != nil {
			dbObj.Properties = map[string]string{"model": *obj.Spec.Ollama.Model}
		}
	}
	...
	if obj.Spec.Gemini != nil {
		...
		if obj.Spec.Gemini.Model != nil {
			dbObj.Properties = map[string]string{"model": *obj.Spec.Gemini.Model}
		}
	}
	...
	if obj.Spec.SMTP != nil {
		...
		dbObj.URL = fmt.Sprintf("smtp://$(username):$(password)@%s:%d/?UseStartTLS=%s&Encryption=%s&Auth=%s&from=%s&to=%s",
			obj.Spec.SMTP.Host, obj.Spec.SMTP.Port,
			strconv.FormatBool(obj.Spec.SMTP.InsecureTLS), obj.Spec.SMTP.Encryption,
			obj.Spec.SMTP.Auth, obj.Spec.SMTP.FromAddress,
	...
	if obj.Spec.Ntfy != nil {
		...
		dbObj.Properties = map[string]string{"topic": obj.Spec.Ntfy.Topic}
	}
	...
	if obj.Spec.Pushbullet != nil {
		...
		dbObj.Properties = map[string]string{"targets": targets}
	}

...
func getCheckStatusHandler(goctx gocontext.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	ctx, err := getDutyCtx(goctx)
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
...
func registerHealthChecks(s *server.MCPServer) {
	...
	We can search health checks via query using the same grammar as catalog search
	...
	Fields:  type | id | name | namespace | labelSelector | tagSelector | status | health | limit | created_at | updated_at | deleted_at | label.* | tag.*
	...
	*value*:  contains match
	...
	labelSelector="team in (payments,orders)"
	label.app=nginx tag.cluster=prod
...
`
	...
	s.AddTool(getCheckStatusTool, getCheckStatusHandler)
	...
	listAllChecksTool := mcp.NewTool("list_all_checks",
		...
		mcp.WithReadOnlyHintAnnotation(true),
		mcp.WithNumber("limit", mcp.Description("Number of checks to return (default: 100)")),
	)

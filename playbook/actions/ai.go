...
func buildPrompt(ctx context.Context, prompt string, spec v1.AIActionContext) (string, error) {
	...
	paragraphs := []string{
		fmt.Sprintf("Config: %s", codeblock(string(lo.FromPtr(pctx.config.Config)))),
...
func formatRelatedConfigs(configs []query.RelatedConfig) string {
	...
	seen := make(map[string]struct{})
	var lines []string
	lines = append(lines, " id | name | type | created at | updated at | changes | status | health")
	lines = append(lines, "--- | ---  | ---  | ---        | ---        | ---     | ---    | ---")
	...
	return fmt.Sprintf("here are the related configs\n\n%s", codeblock(strings.Join(lines, "\n")))
}
...
func formatRelatedConfigsGraph(configs []query.RelatedConfig) string {
	...
	var lines []string
	lines = append(lines, "parent | child")
	lines = append(lines, "--- | ---")
	for _, c := range configs {
	...
	return fmt.Sprintf("here's how these configs are related\n\n%s", codeblock(strings.Join(lines, "\n")))
}
...
func codeblock(code any) string {
	const format = "```\n%s\n```"
	...
	marshalled, err := json.Marshal(code)
	if err != nil {
		logger.Errorf("failed to marshal code block for prompt: %v", err)
		return "<bad-json>"
	}

	return fmt.Sprintf(format, string(marshalled))
}

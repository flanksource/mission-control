...
| ------------------- | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| JWT signing keypair | `auth/signing`; flag `--signing-private-key`; `dutyAPI.DefaultConfig.Postgrest.JWTSecret` | Private key is loaded from `.mission-control-signing-key.pem` or `--signing-private-key`; if missing, an ephemeral in-memory key is generated | Signs internal PostgREST/session JWTs, basic login cookie JWTs, plugin invocation JWTs, and embedded OIDC access/ID JWTs | PostgREST receives the derived public JWK in `Postgrest.JWTSecret`, enforces `jwt-aud=mission-control-postgrest`, and verifies RS256 tokens with the public key. |

...
All keys below have prefix `notification.recovery.`. Durations use Go syntax (`720h`, not `30d`). Schedule intervals are read when jobs are registered; restart/re-register jobs to change intervals. Other settings are read each invocation.

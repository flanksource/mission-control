...
function StageCellPill({
  ...
  return (
    <div
      className={`flex items-center gap-[1.2mm] rounded-full border pl-[0.5mm] pr-[1mm] py-[0.5mm] ${spec.cellBg}`}
      style={{ borderColor: stageCellBorderColor(actor) }}
    >
      <ActorAvatar actor={actor} size="xs" />
      <span className="min-w-0 flex-1 text-xs leading-none text-slate-800 truncate">{actor}</span>
...
function StageCard({ stage, referenceDate }: { stage: PromotionStage; referenceDate: string }) {
  ...
  return (
    <div className="flex-1 min-w-[28mm] rounded-[1.5mm] border border-slate-200 bg-slate-50 px-[1.8mm] py-[1.6mm]">
      <div className="flex items-center gap-[1mm]">
        <div className="text-sm font-semibold text-slate-800">{stage.name}</div>
        {dateLabel && (
          <div className="ml-auto text-xs text-slate-400">{dateLabel}</div>
        )}
      </div>
...
function PromotionPipeline({ group }: { group: PromotionGroup }) {
  ...
  return (
    <div className="rounded-[2mm] border border-slate-200 bg-white px-[2.5mm] py-[2mm] mb-[3mm]" style={NO_BREAK_STYLE}>
      <div className="flex items-center gap-[1.2mm] mb-[2mm]">
        <Icon name={getChangeEventIconName({ changeType: 'Promotion', typedChange: { kind: 'Promotion/v1' } })} size={10} className="text-indigo-600" />
        <span className="text-sm font-semibold text-slate-800">{group.artifact}</span>
        <Badge
          variant="custom"
          size="xxs"
          shape="rounded"
        ...
        <span className="ml-auto text-xs text-slate-400">{formatDateTime(group.latestDate)}</span>
      </div>
      ...
      <div className="flex items-stretch gap-[2mm]">

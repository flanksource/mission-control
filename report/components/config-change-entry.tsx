...
import { formatEntryDate, humanizeSize, type TimeBucketFormat } from './utils.ts';
...
export function ActorIdentity({ actor }: { actor: string }) {
  return (
    <span className="inline-flex max-w-full items-center justify-end gap-[0.7mm] text-[7.5pt] leading-[9pt] text-slate-500">
      <ActorAvatar actor={actor} size="xs" />
      <span className="break-words text-right">{actor}</span>
    </span>
  );
}
...
export function ChangeEntry({ change, dateFormat, hideConfigName }: { change: ConfigChange; dateFormat: TimeBucketFormat; hideConfigName?: boolean }) {
  ...
  return (
    <div className="flex items-start gap-[1.5mm] py-[0.55mm] border-b border-slate-50 last:border-b-0 text-[8.5pt] leading-[10pt]">
      ...
      <div className="shrink-0 flex items-center gap-[0.8mm] pt-[0.25mm]">
        <SeverityIcon severity={severity} />
        <ChangeIcon change={change} />
      </div>
      <div className="flex-1 min-w-0 flex items-start gap-[1.4mm]">
        <div className="min-w-0 flex-1 flex flex-wrap items-center gap-[0.8mm] text-[8.5pt] leading-[10pt] text-slate-700">

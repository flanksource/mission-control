...
`
	...
	ginkgo.Describe("LoadDefaultSettings", func() {
		ginkgo.It("loads embedded defaults", func() {
			...
			Expect(s.Thresholds.StaleDays).To(Equal(0))
			Expect(s.Thresholds.ReviewOverdueDays).To(Equal(0))
			...
			deploymentFailedFilter := ""
			for _, mapping := range s.CategoryMappings {
				if mapping.Category == "deployment.failed" {
					deploymentFailedFilter = mapping.Filter
					break
				}
			}
			Expect(deploymentFailedFilter).To(Equal(`changeType == "CodeDeployment" && severity != "info"`))
		})
	})
...
`
	...
	ginkgo.Describe("Options threshold methods", func() {
		ginkgo.It("returns zero when no settings (disabled)", func() {
			opts := Options{}
			Expect(opts.StaleDays()).To(Equal(0))
			Expect(opts.ReviewOverdueDays()).To(Equal(0))
		})
		...
		ginkgo.It("returns zero when settings thresholds are zero (disabled)", func() {

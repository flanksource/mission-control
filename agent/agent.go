...
func generateAgent(ctx context.Context, body api.GenerateAgentRequest) (*api.GeneratedAgent, error) {
	...
	token, _, err := db.CreateAccessToken(ctx, person.ID, "default", password, lo.ToPtr(time.Hour*24*365), nil, body.AutoRenew)
	if err != nil {
...
func generateToken(ctx context.Context, body api.GenerateTokenRequest) (*api.GeneratedToken, error) {
	...
	token, _, err := db.CreateAccessToken(ctx, lo.FromPtr(agent.PersonID), "default", password, lo.ToPtr(time.Hour*24*365), nil, body.AutoRenew)
	if err != nil {
		return nil, fmt.Errorf("failed to create a new access token: %w", err)
	}

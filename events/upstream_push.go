...
func (t *pushToUpstreamEventHandler) Run(ctx context.Context, tx *gorm.DB, event api.Event) error {
	...
	if err := tx.WithContext(ctx).Where("id = ?", pushQueueID).Find(&changelogs).Error; err != nil {
	...
	for tablename, itemIDs := range groupChangelogsByTables(changelogs) {
		switch tablename {
		case "components":
			if err := tx.WithContext(ctx).Where("id IN ?", itemIDs).Find(&upstreamMsg.Components).Error; err != nil {
				return fmt.Errorf("error fetching components; %w", err)
			}

		case "canaries":
			if err := tx.WithContext(ctx).Where("id IN ?", itemIDs).Find(&upstreamMsg.Canaries).Error; err != nil {
				return fmt.Errorf("error fetching canaries; %w", err)
			}

		case "checks":
			if err := tx.WithContext(ctx).Where("id IN ?", itemIDs).Find(&upstreamMsg.Checks).Error; err != nil {
				return fmt.Errorf("error fetching checks; %w", err)
			}

		case "config_analysis":
			if err := tx.WithContext(ctx).Where("id IN ?", itemIDs).Find(&upstreamMsg.ConfigAnalysis).Error; err != nil {
				return fmt.Errorf("error fetching config_analysis; %w", err)
			}

		case "config_changes":
			if err := tx.WithContext(ctx).Where("id IN ?", itemIDs).Find(&upstreamMsg.ConfigChanges).Error; err != nil {
				return fmt.Errorf("error fetching config_changes; %w", err)
			}

		case "config_items":
			if err := tx.WithContext(ctx).Where("id IN ?", itemIDs).Find(&upstreamMsg.ConfigItems).Error; err != nil {
				return fmt.Errorf("error fetching config_items; %w", err)
			}

		case "check_statuses":
			compositeKeys := splitKeys(itemIDs)
			if err := tx.WithContext(ctx).Where(`(check_id, "time") IN ?`, compositeKeys).Find(&upstreamMsg.CheckStatuses).Error; err != nil {
				return fmt.Errorf("error fetching check_statuses; %w", err)
			}

		case "config_component_relationships":
			compositeKeys := splitKeys(itemIDs)
			if err := tx.WithContext(ctx).Where("(component_id, config_id) IN ?", compositeKeys).Find(&upstreamMsg.ConfigItems).Error; err != nil {
				return fmt.Errorf("error fetching config_component_relationships; %w", err)
			}

		case "component_relationships":
			compositeKeys := splitKeys(itemIDs)
			if err := tx.WithContext(ctx).Where("(component_id, relationship_id, selector_id) IN ?", compositeKeys).Find(&upstreamMsg.ComponentRelationships).Error; err != nil {
				return fmt.Errorf("error fetching component_relationships; %w", err)
			}

		case "config_relationships":
			keys := splitKeys(itemIDs)
			if err := tx.WithContext(ctx).Where("(related_id, config_id, selector_id) IN ?", keys).Find(&upstreamMsg.ConfigRelationships).Error; err != nil {

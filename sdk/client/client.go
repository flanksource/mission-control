// Package client provides Faro's lightweight Mission Control HTTP client.
package client
...
func (c *Client) InvokePluginOperation(name, operation, configID string, params json.RawMessage) ([]byte, error) {
	...
	r, err := req.Post(c.apiPath("/api/plugins/"+url.PathEscape(name)+"/invoke/"+url.PathEscape(operation)), params)
	if err != nil {
		return nil, err
	}

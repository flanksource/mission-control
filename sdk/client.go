...
func New(serverURL, token string, opts ...ClientOption) *Client {
	...
	if tokenProvider == nil && token != "" {
		tokenProvider = StaticTokenProvider(token)
	}
	if tokenProvider != nil {
		client = client.Use(bearerAuthMiddleware(tokenProvider))
	}

	return &Client{
		Client:        httpobservability.Apply(client),
...
func bearerAuthMiddleware(provider TokenProvider) func(stdhttp.RoundTripper) stdhttp.RoundTripper {

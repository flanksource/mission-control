...
func parsePlaybookArgs(ctx context.Context, args []string) (*models.Playbook, *playbook.RunParams, error) {
	...
	if f, err := os.Open(clientcmd.ParamFile); err == nil {
...
func runLocalPlaybook(cmd *cobra.Command, args []string) error {
	logger.UseSlog()
...
func init() {
	clientcmd.Run.PersistentPreRun = PreRun
	clientcmd.Run.Flags().IntVar(&debugPort, "debug-port", -1, "Start an HTTP server to use the /debug routes, Use -1 to disable and 0 to pick a free port")
	clientcmd.LocalRunHandler = runLocalPlaybook

	clientcmd.Playbook.AddCommand(Submit)
}

...
func ServerFlags(flags *pflag.FlagSet) {
	...
	flags.StringVar(&api.RemotePluginHostGRPCAddress, "plugin-host-grpc-address", api.RemotePluginHostGRPCAddress, "Address remote plugins use to reach this instance's plugin HostService gRPC back-channel. Default for spec.hostGRPCAddress.")
	flags.StringVar(&api.PluginHostTLSCertFile, "plugin-host-tls-cert", "", "TLS certificate file for the plugin HostService gRPC server (enables TLS with --plugin-host-tls-key)")
	flags.StringVar(&api.PluginHostTLSKeyFile, "plugin-host-tls-key", "", "TLS private key file for the plugin HostService gRPC server")
	flags.StringVar(&api.PluginHostTLSCAFile, "plugin-host-tls-ca", "", "PEM CA bundle advertised to remote plugins to verify the HostService TLS certificate (defaults to system roots)")
	flags.StringVar(&api.PluginHostTLSClientCAFile, "plugin-host-tls-client-ca", "", "PEM CA bundle to require and verify remote plugin client certificates on the HostService gRPC server (enables mTLS)")
	flags.StringVar(&api.PluginHostClientCertFile, "plugin-host-client-cert", "", "Client certificate Mission Control presents when dialing remote plugins that require mTLS")
	flags.StringVar(&api.PluginHostClientKeyFile, "plugin-host-client-key", "", "Client private key for --plugin-host-client-cert")
	...
	flags.BoolVar(&echo.UIEnabled, "ui", false, "Serve the catalog explorer UI at /ui")

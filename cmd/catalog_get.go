...
// buildCatalogGetOptionsFromFlags parses the flag map handed over by clicky's
// entity get action into a catalogGetOptions.
func buildCatalogGetOptionsFromFlags(flags map[string]string) (catalogGetOptions, error) {
	since := flags["since"]
	if since == "" {
		since = "7d"
	}
	...
	switch query.RelationDirection(direction) {
	case query.All, query.Incoming, query.Outgoing:
		opts.Direction = query.RelationDirection(direction)
	default:
		return catalogGetOptions{}, fmt.Errorf("invalid --direction %q: must be all, incoming, or outgoing", direction)
	}

	if raw := flags["depth"]; raw != "" {
		d, err := strconv.Atoi(raw)
		if err != nil {
			return catalogGetOptions{}, fmt.Errorf("invalid --depth %q: %w", raw, err)
		}
		if d > 0 {
			opts.Depth = &d
		}
...
func resolveConfigsForCommand(ctx context.Context, args []string) ([]models.ConfigItem, error) {
	return resolveConfigs(ctx, args, 0)
}
...
func resolveConfigs(ctx context.Context, args []string, limit int) ([]models.ConfigItem, error) {
	...
	for _, arg := range args {
		req := parseQuery([]string{arg})
		if limit > 0 {
			req.Limit = limit
		}
		response, err := query.SearchResources(ctx, req)
		if err != nil {
			return nil, fmt.Errorf("search failed for %q: %w", arg, err)
		}
	...
	if len(ids) == 0 {
		return nil, fmt.Errorf("no config found matching queries")
	}

	var configs []models.ConfigItem
	for _, id := range ids {
		config, err := query.GetCachedConfig(ctx, id)
		if err != nil {
			return nil, fmt.Errorf("failed to get config %s: %w", id, err)
		}
		if config != nil {
...
type CatalogGetResult struct {
	...
	AccessLogs        []accessLogRow               `json:"access_logs,omitempty"`
...
// CatalogGetResults wraps multiple CatalogGetResult items so the catalog get
// command can return more than one match from a query.
type CatalogGetResults struct {
	Results []CatalogGetResult `json:"results"`
}

func (r CatalogGetResults) Pretty() api.Text {
	t := clicky.Text("")
	t = t.Add(clicky.Text(fmt.Sprintf("%d matching configs", len(r.Results)), "font-bold text-gray-700"))
	for i := range r.Results {
		t = t.NewLine().HR().NewLine().Append(r.Results[i].Pretty())
	}
	return t
}
...
type accessLogRow struct {
	...
	ExternalUserID uuid.UUID      `gorm:"column:external_user_id" json:"external_user_id"`
	UserName       string         `gorm:"column:user_name" json:"user_name"`
	CreatedAt      time.Time      `gorm:"column:created_at" json:"created_at"`
	MFA            bool           `gorm:"column:mfa" json:"mfa"`
	Count          *int           `gorm:"column:count" json:"count,omitempty"`
	Properties     map[string]any `gorm:"column:properties;serializer:json" json:"properties,omitempty"`
}

func buildCatalogGetResult(ctx context.Context, config *models.ConfigItem, opts catalogGetOptions, sinceTime time.Time) (*CatalogGetResult, error) {
	id := config.ID
	result := &CatalogGetResult{
		ConfigItem:     *config,
		since:          opts.Since,
		showConfigJSON: opts.Sections.ConfigJSON,
	}
	var err error
	...
	if opts.Sections.AccessLogs {
		if err := ctx.DB().
			...
			Where("config_access_logs.config_id = ?", id).
			Where("config_access_logs.created_at >= ?", sinceTime).
			Order("config_access_logs.created_at DESC").
			Limit(50).
			Scan(&result.AccessLogs).Error; err != nil {
			return nil, fmt.Errorf("failed to get access logs: %w", err)
		}
	}

	if opts.PlaybookRuns {
...
func (r CatalogGetResult) Pretty() api.Text {
	...
	if r.showConfigJSON && r.ConfigItem.Config != nil && *r.ConfigItem.Config != "" {
	...
	if len(r.AccessLogs) > 0 {
		t = t.NewLine().Append(clicky.Collapsed(
			fmt.Sprintf("Access Logs since %s (%d)", sinceLabel, len(r.AccessLogs)),
			api.NewTableFrom(r.AccessLogs),
		))
	}
...
func (r accessLogRow) Row() map[string]any {
	mfa := clicky.Text("no", "text-gray-500")
	if r.MFA {
		mfa = clicky.Text("yes", "text-green-600")
	}
	count := clicky.Text("-", "text-gray-400")
	if r.Count != nil {
		count = clicky.Text(fmt.Sprintf("%d", *r.Count))
	}
	return map[string]any{
		"User":  clicky.Text(r.UserName, "font-bold"),
		"When":  api.Human(time.Since(r.CreatedAt), "text-gray-600"),
		"MFA":   mfa,
		"Count": count,
	}
}
